...
//
// BUG FIX (master prompt Bug #2):
//   Changed Cache-Control from 'no-store' to 's-maxage=1800, stale-while-revalidate=3600'
//   This caches each sheet for 30 minutes at Vercel's edge — biggest single performance win.
...
async function handler(req, res) {
  ...
  // Now Vercel edge caches each sheet for 30 min; stale responses served during refresh.
  res.setHeader('Cache-Control', 's-maxage=1800, stale-while-revalidate=3600');

  if (req.method === 'OPTIONS') return res.status(204).end();
  if (req.method !== 'GET') return res.status(405).json({ error: 'Method not allowed' });

  const sheet = req.query.sheet || req.query.type;
