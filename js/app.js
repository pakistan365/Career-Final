...
function openCardPost(id, type) {
  // Admission cards: find the item and navigate to its applyLink
  if (type === 'admission') {
    const admissions = window.CMS_DATA.UniversityAdmissions || window.CMS_DATA.universityAdmissions || [];
    const item = admissions.find(u => String(u.id) === String(id));
    const link = item && (item.applyLink || item.apply_link);
    if (link && link !== '#') {
      window.open(safeUrl(link), '_blank', 'noopener,noreferrer');
    } else {
      window.location.href = 'university-admissions.html';
    }
    return;
  }
...
function cardUniversityAdmission(u) {
  ...
  const imgHTML    = src ? `<img src="${escapeHtml(src)}" alt="${escapeHtml(u.universityName||'')}" loading="lazy" onerror="this.src='${fallback}'">` : `<div class="card-img-placeholder">🎓</div>`;
  const entryBadge = u.entryTestRequired ? `<span class="card-tag" style="background:#fef3c7;color:#92400e;border-color:#fde68a"><i class="fa fa-pen-nib fa-xs"></i> ${escapeHtml(u.entryTestName || 'Entry Test')}</span>` : '';
  const meritBtn   = u.meritListUrl ? `<a class="btn btn-secondary" href="${escapeHtml(u.meritListUrl)}" target="_blank" rel="noopener">Merit List</a>` : '';
  ...
  <div class="card" data-id="${u.id}" data-type="admission" role="button" tabindex="0" aria-label="View ${escapeHtml(u.universityName || '')} admissions">
    ...
    <div class="card-body">
      ...
      <p class="card-desc">${escapeHtml(u.shortDescription || u.programs || '')}</p>
    ...
    <div class="card-footer">
      ${meritBtn}
      <a class="btn btn-secondary" href="opportunity.html?type=admission&id=${encodeURIComponent(u.id)}">Details <i class="fa fa-info-circle fa-xs"></i></a>
      <a class="btn btn-primary" href="${escapeHtml(u.applyLink || u.apply_link || '#')}" target="_blank" rel="noopener">Apply <i class="fa fa-arrow-right fa-xs"></i></a>
    </div>
  </div>`;
}
