...
function openCardPost(id, type) {
  // Admission cards: navigate to opportunity detail page
  if (type === 'admission') {
    window.location.href = `opportunity.html?type=admission&id=${encodeURIComponent(id)}`;
    return;
  }
  // Blog cards: route to blog-post.html
  if (type === 'blog') {
    window.location.href = `blog-post.html?id=${encodeURIComponent(id)}`;
    return;
  }
...
// ── Card renderers ───────────────────────────────────────────
function cardUniversityAdmission(u) {
  const deadline   = u.deadline || u.lastDateToApply || '';
  const img        = u.imageUrl || u.image_url || '';
  const fallback   = 'https://placehold.co/320x160/e0f2fe/0284c7?text=' + encodeURIComponent(u.shortName || 'UNI');
  const imgHTML    = img
    ? `<img src="${escapeHtml(img)}" alt="${escapeHtml(u.universityName||'')}" loading="lazy" onerror="this.src='${fallback}'">`
    : `<div class="card-img-placeholder" style="display:flex;align-items:center;justify-content:center;height:100%;font-size:2.5rem;background:linear-gradient(135deg,#e0f2fe,#bae6fd)">🎓</div>`;
  const entryBadge = u.entryTestRequired
    ? `<span class="card-tag" style="background:#fef3c7;color:#92400e;border-color:#fde68a"><i class="fa fa-pen-nib fa-xs"></i> ${escapeHtml(u.entryTestName || 'Entry Test')}</span>`
    : '';
  const fav  = isFav(u.id, 'admission');
  const favAttrs = getFavButtonStateAttrs(fav);
  const detailUrl = `opportunity.html?type=admission&id=${encodeURIComponent(u.id)}`;
  const applyUrl  = escapeHtml(u.applyLink || u.apply_link || '#');

  return `
  <div class="card" data-id="${u.id}" data-type="admission" role="button" tabindex="0" aria-label="View ${escapeHtml(u.universityName || '')} admissions">
    <div class="card-img">
      ${imgHTML}
      ${deadline ? urgencyBadge(deadline) : ''}
      <button class="btn-fav${fav ? ' active' : ''}" data-id="${u.id}" data-type="admission" data-title="${escapeHtml(u.universityName||'')}" onclick="event.stopPropagation();toggleFav('${u.id}','${escapeHtml(u.universityName||'')}','admission');this.classList.toggle('active')" ${favAttrs}><i class="fa fa-bookmark"></i></button>
    </div>
    <div class="card-body">
      <div class="card-meta">
        ${u.universityType ? `<span class="card-tag">${escapeHtml(u.universityType)}</span>` : ''}
        ${u.degreeLevel    ? `<span class="card-tag">${escapeHtml(u.degreeLevel)}</span>`    : ''}
        ${entryBadge}
      </div>
      <h3 class="card-title">${escapeHtml(u.universityName || '')}</h3>
      <p class="card-desc">${escapeHtml(truncateText(u.shortDescription || u.programs || '', 100))}</p>
      ${u.city ? `<p class="card-org"><i class="fa fa-map-marker-alt"></i> ${escapeHtml(u.city)}${u.province ? ', ' + escapeHtml(u.province) : ''}</p>` : ''}
      <div class="card-details">
        ${u.minimumPercentage !== null && u.minimumPercentage !== undefined ? `<span><i class="fa fa-percent"></i> ${u.minimumPercentage}% min</span>` : ''}
        ${deadline ? `<span><i class="fa fa-calendar-alt"></i> ${formatDate(deadline)}</span>` : ''}
        ${u.fee ? `<span><i class="fa fa-money-bill-wave"></i> ${escapeHtml(u.fee)}</span>` : ''}
      </div>
    </div>
...
function cardBlog(b) {
  ...
  <div class="card" data-id="${b.id}" data-type="blog" role="button" tabindex="0" aria-label="Read ${escapeHtml(b.title)}">
    ...
    <div class="card-footer">
      <a class="btn btn-primary" href="blog-post.html?id=${encodeURIComponent(b.id)}">Read More <i class="fa fa-arrow-right"></i></a>
    ...
    <div id="resourcePreviewModal" class="resource-preview-modal" role="dialog" aria-modal="true" aria-label="Document preview">
      <div class="resource-preview-head">
        ...
        <div style="display:flex;gap:.5rem;align-items:center">
          <a id="resourcePreviewDownload" href="#" download class="btn btn-primary" target="_blank" rel="noopener noreferrer" style="font-size:.82rem;padding:.4rem .85rem"><i class="fa fa-download"></i> Download</a>
      ...
      <img id="resourcePreviewImg" style="max-width:100%;max-height:75vh;object-fit:contain;display:none;border-radius:8px;margin:0 auto;display:none" alt="Preview"/>
      <iframe id="resourcePreviewFrame" class="resource-preview-frame" loading="lazy" referrerpolicy="no-referrer"></iframe>
    </div>`;
  document.body.appendChild(wrapper);
}

function openResourcePreview(url, title, mediaType) {
  ensureResourcePreviewModal();
  const frame = document.getElementById('resourcePreviewFrame');
  const img   = document.getElementById('resourcePreviewImg');
  const dl    = document.getElementById('resourcePreviewDownload');
  const hint  = document.getElementById('resourcePreviewHint');
  const safe  = safeUrl(url);
  document.getElementById('resourcePreviewTitle').textContent = text(title || 'Preview');
  dl.href = safe;
  dl.setAttribute('download', title || 'document');
  // Reset both
  frame.style.display = 'none'; frame.src = 'about:blank';
  img.style.display   = 'none'; img.src   = '';
  if (mediaType === 'image') {
    img.src = safe; img.style.display = 'block';
    hint.textContent = '';
  } else {
    const hostHint = isTeraBoxUrl(safe)
      ? 'TeraBox links may block embedded preview. Try downloading directly.'
      : 'If this file does not render, use the Download button above.';
    hint.textContent = hostHint;
    frame.src = safe; frame.style.display = 'block';
  }
...
function sortItems(items, sort) {
  ...
  if (sort === 'deadline') {
    const _now = Date.now();
    const _getMs = i => {
      const raw = i.deadline || i.registrationDeadline || i.registration_deadline
                || i.testDate || i.test_date || '';
      if (!raw) return null;
      const ms = new Date(raw).getTime();
      return Number.isFinite(ms) ? ms : null;
    };
    const _active  = arr.filter(i => { const ms = _getMs(i); return !ms || ms >= _now; });
    const _expired = arr.filter(i => { const ms = _getMs(i); return ms && ms < _now; });
    _active.sort( (a, b) => (_getMs(a) || 9e15) - (_getMs(b) || 9e15));
    _expired.sort((a, b) => _getMs(b) - _getMs(a));
...
const container = document.getElementById(containerId);
  ...
  container.innerHTML = latestRows.map((item) => {
    ...
    const urgencyBadgeHTML = (type !== 'book' && d >= 0 && d <= urgentThresholdDays)
      ...
      : '';
    const itemUrl = type === "blog" ? `blog-post.html?id=${encodeURIComponent(item.id)}` : getCardDetailsUrl(item.id, type, item.title);
    return `<a class="home-latest-item" href="${itemUrl}"><strong>${escapeHtml(item.title || 'Untitled')}</strong><span>${escapeHtml(groupName)} • ${dateLabel}: <span class="deadline-date">${formatDate(dateValue)}</span></span>${urgencyBadgeHTML}</a>`;
  }).join('');
...
function getOverlay() {
  ...
  if (!el) {
    ...
    // Note: visibility is controlled via CSS class — no inline display needed
...
function showOverlay() {
  const o = getOverlay();
  // No display toggle needed — visibility+opacity handles show/hide
...
function hideOverlay() {
  ...
  // No setTimeout cleanup needed — visibility:hidden keeps it inert
}
...
function openMenu() {
  ...
  // Make background content inert (inaccessible to Tab/screen readers)
  const main = document.querySelector('main, .home-shell, .page-section, #mainContent, .hero, section');
  if (main && 'inert' in HTMLElement.prototype) main.inert = true;

  showOverlay();

  // Focus first item in drawer after animation
  setTimeout(() => {
    const first = navLinks.querySelector('a[href], button:not([disabled])');
    if (first) first.focus({ preventScroll: true });
  }, 80);

  // Swipe left to close
  let _sx = 0;
  navLinks._onTouchStart = e => { _sx = e.touches[0].clientX; };
  navLinks._onTouchMove  = e => { if (_sx - e.touches[0].clientX > 60) closeMenu(); };
  navLinks.addEventListener('touchstart', navLinks._onTouchStart, { passive: true });
  navLinks.addEventListener('touchmove',  navLinks._onTouchMove,  { passive: true });
}
...
function closeMenu() {
  ...
  if (navLinks) {
    ...
    navLinks.querySelectorAll('.has-dropdown.accordion-open')
            .forEach(el => el.classList.remove('accordion-open'));
    // Remove swipe listeners
    if (navLinks._onTouchStart) navLinks.removeEventListener('touchstart', navLinks._onTouchStart);
    if (navLinks._onTouchMove)  navLinks.removeEventListener('touchmove',  navLinks._onTouchMove);
    navLinks._onTouchStart = null;
    navLinks._onTouchMove  = null;
  }
...
function _menuFocusTrap(event) {
  if (!menuOpen || event.key !== 'Tab') return;
  const navLinks = document.getElementById('navLinks');
  if (!navLinks) return;
  const focusable = Array.from(navLinks.querySelectorAll(
...
function initMenu() {
  ...
  // Keyboard Enter/Space for accordion dropdowns in mobile drawer
  navLinks.addEventListener('keydown', (kev) => {
    if (kev.key !== 'Enter' && kev.key !== ' ') return;
    const anchor = kev.target.closest('a');
    if (!anchor) return;
    const dropItem = anchor.closest('.has-dropdown');
    const isTrigger = dropItem && anchor.parentElement === dropItem && !anchor.closest('.dropdown');
    if (isTrigger && isMobile()) {
      kev.preventDefault();
      const wasOpen = dropItem.classList.contains('accordion-open');
      navLinks.querySelectorAll('.has-dropdown.accordion-open')
               .forEach(el => el.classList.remove('accordion-open'));
      if (!wasOpen) dropItem.classList.add('accordion-open');
    }
  });
  ...
  // Mark active sidebar links based on current page
...
// ── Init on DOM ready ────────────────────────────────────────
document.addEventListener('DOMContentLoaded', () => {
  // Measure scrollbar width once so body.nav-menu-open padding-right prevents layout shift
  const sbw = window.innerWidth - document.documentElement.clientWidth;
  if (sbw > 0) document.documentElement.style.setProperty('--scrollbar-width', sbw + 'px');
