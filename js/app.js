...
function openCardPost(id, type) {
  ...
  // Blog cards: route to unified opportunity detail page
  if (type === 'blog') {
    window.location.href = `opportunity.html?type=blog&id=${encodeURIComponent(id)}`;
    return;
  }
...
function cardUniversityAdmission(u) {
  ...
  const src = getCardImage(u, 'admission') || u.imageUrl || u.image_url || '';
  const imgHTML = cardImageHTML(src, u.universityName || '');
  const entryBadge = u.entryTestRequired
    ...
    : '';
  const detailUrl = getCardDetailsUrl(u.id, 'admission', u.universityName);
  ...
  <div class="card" data-id="${u.id}" data-type="admission" role="button" tabindex="0" aria-label="View ${escapeHtml(u.universityName || '')} admissions">
    ...
    <div class="card-footer">
      <a class="btn btn-primary" href="${detailUrl}">View Details <i class="fa fa-arrow-right"></i></a>
      <button class="btn-share" onclick="shareOpportunity('${escapeJsSingleQuote(normalizeItemId(u.id))}','admission','${escapeJsSingleQuote(u.universityName||'')}',this)" ${shareButtonAttrs(u.universityName)}><i class="fa fa-share-nodes"></i></button>
      <button class="btn-fav ${fav?'active':''}" onclick="handleFav('${escapeJsSingleQuote(String(u.id))}','${escapeJsSingleQuote(u.universityName||'')}','admission',this)" ${getFavButtonStateAttrs(fav)}><i class="fa${fav?'s':'r'} fa-bookmark"></i><span class="visually-hidden">${fav?'Saved':'Save'}</span></button>
    </div>
  </div>`;
}
...
function cardBlog(b) {
  ...
  <div class="card" data-id="${b.id}" data-type="blog" role="button" tabindex="0" aria-label="Read ${escapeHtml(b.title)}">
    ...
    <div class="card-footer">
      <a class="btn btn-primary" href="opportunity.html?type=blog&id=${encodeURIComponent(b.id)}">Read More <i class="fa fa-arrow-right"></i></a>
...
const container = document.getElementById(containerId);
  ...
  container.innerHTML = latestRows.map((item) => {
    ...
    const urgencyBadgeHTML = (type !== 'book' && d >= 0 && d <= urgentThresholdDays)
      ...
      : '';
    const itemUrl = getCardDetailsUrl(item.id, type, item.title);
...
function initCounters() {
  if (window.__careerPkCountersReady) return;
  window.__careerPkCountersReady = true;
  updateCMSCounts();
