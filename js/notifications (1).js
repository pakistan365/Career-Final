...
// ════════════════════════════════════════════════════════════
// NAVIGATE TO FULL OPPORTUNITY PAGE
// ════════════════════════════════════════════════════════════
// Notification cards open the shared opportunity.html detail page
// (same header, sidebar and footer as Jobs/Scholarships/etc.) rather
// than only the in-page quick-view modal.
function goToNotification(notifId) {
  if (!notifId) return;
  window.location.href = 'opportunity.html?type=notification&id=' + encodeURIComponent(notifId);
}
...
function renderNotifications() {
  ...
  try {
    html = filteredNotifications.map(n => {
      try { return renderNotificationCard(n); }
      catch (cardErr) { console.error('[Notifications] Failed to render card', n && n.id, cardErr); return ''; }
    }).join('');
  } catch (renderErr) {
    console.error('[Notifications] Failed to render grid', renderErr);
    html = '';
  }
  grid.innerHTML = html;
...
function renderNotificationCard(notif) {
  ...
  return `
    <div class="notif-card" onclick="goToNotification('${escHtml(notif.id)}')" role="button" tabindex="0"
         onkeydown="if(event.key==='Enter')goToNotification('${escHtml(notif.id)}')">
      ${image
...
function updateRecentList() {
  ...
  recentList.innerHTML = recent.map(n => `
    <div style="padding:.5rem .6rem;background:var(--bg-secondary,#f8fafc);border-radius:6px;border-left:3px solid var(--primary,#0f766e);cursor:pointer;transition:transform .15s"
         onclick="goToNotification('${escHtml(n.id)}')"
...
function renderMoreFromCategory(currentId, category) {
  ...
  moreList.innerHTML = more.map(n => `
    <div class="related-item" onclick="goToNotification('${escHtml(n.id)}')">
...
function openRequestedNotification() {
  ...
  setTimeout(() => goToNotification(id), 0);
}
